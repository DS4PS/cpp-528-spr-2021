...
`r emo::ji("construction")`: heads up, this files does not exist yet. If you want it to exist, go down to "Lab Instructions" and look for ["Data Concordance File"](../../data/LTDB-DATA-DICTIONARY.xlsx) where you'll make it using a combination of Excel and the [codebook](../../data/LTDB-codebook.pdf) for this week's lab.
...
Download the [DATA CONCORDANCE FILE](../../data/LTDB-DATA-DICTIONARY.xlsx) that was created from the steps described above. You will see that it has all of the information about variable changes over time, but it has incomplete meta-data that describes the variables. 


Using the [LTDB Codebook](../../data/LTDB-codebook.pdf) copy and paste the Census definitions of each field into the "description" column. 
